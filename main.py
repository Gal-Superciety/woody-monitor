...
def get_swap_usd_value(quote_token: str, quote_amount: float) -> float:
...
def merge_token_items(items: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    for item in items:
        token = item["token"]
        merged[token] = merged.get(token, 0.0) + safe_float(item["amount"])
    return merged
...
def detect_pair_and_dex(tx: dict) -> Tuple[str, str]:
    ...
    quote = quote_candidates[0] if quote_candidates else "?"
    pair = f"WOODY / {quote}"
    return pair, dex
...
def choose_title(tx_type: str) -> str:
    if tx_type == "BUY":
...
def build_message(tx_hash: str, parsed: Dict[str, Any]) -> str:
    ...
    title = choose_title(parsed["type"])
...
def start_caption() -> str:
    return (
        ...
        "Live tracking for the WOODY ecosystem.\n\n"
        "This bot monitors:\n"
        "• Price\n"
        "• Liquidity status\n"
        "• Holders\n"
        "• Real-time WOODY transactions\n"
